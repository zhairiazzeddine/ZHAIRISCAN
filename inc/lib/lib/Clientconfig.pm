...
my $res1=join("|", @res);
my $x1="/usr/share/applications/zhairiscan.desktop";
my (@l22, @l22bak);
          ...
          if (($act && ($act ne "options") && !$op) || ($act eq "set" && !$vl)) {
            ...
            if ($act eq "set") {
              if ((grep/^$op$/, @ValueOn) && ($vl ne "on")) {
                ...
                if ($op eq "interactive") {
                  if (-e $x1) {
                    system("sed -i 's/zhairiscan;/zhairiscan --interactive;/g' $x1");
            ...
            ####################################################################################################
              ...
              if ($op eq "all") {        
                ...
                close FH;
                system("sed -i 's/zhairiscan --interactive;/zhairiscan;/g' $x1");
