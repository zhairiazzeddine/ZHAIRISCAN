...
sub uninstall {
  ...
  chomp ($resp);
  if ($resp=~/(Y|y)/) {
    my @uninstall=($scriptbash, $script, "$scriptCompletion/zhairiscan", "/usr/share/applications/zhairiscan.desktop");
    for (@uninstall) {
      print $c[10]."[i] $ZT[8] $_... ";
      unlink $_ if -e $_;
	...
	my @deleteIcons=( "/usr/share/icons/hicolor/16x16/apps/zhairiscan-menu.png", 
	                  "/usr/share/icons/hicolor/22x22/apps/zhairiscan-menu.png", 
					  "/usr/share/icons/hicolor/24x24/apps/zhairiscan-menu.png", 
					  "/usr/share/icons/hicolor/48x48/apps/zhairiscan-menu.png", 
					  "/usr/share/icons/hicolor/256x256/apps/zhairiscan-menu.png"
					 );
    for (@deleteIcons) {
      print $c[10]."[i] $ZT[8] $_... ";
      unlink $_ if -e $_;
