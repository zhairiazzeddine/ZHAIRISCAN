...
#############################################################################################################################
## INTRODUCTION #############################################################################################################
#   This script is Copyright (c) 2024 ZHAIRI AZZEDDINE see License.txt
#   [::] SCRIPT NAME:  zhairiscan scanner
#   [::] AUTOR:        ZHAIRI AZZEDDINE
#   [::] FB:           https://facebook.com/ZHAIRI.AZZEDDINE
#   [::] GIT:          https://github.com/ZHAIRIAZZEDDINE
#
## DESCRIPTION ##############################################################################################################
...
## DEFINE ARRAYS
my (@proxies, @exploits, @motors, @target_urls, @dorks, @targets, @zhairiscans, $isscan);
...
for my $targ(@targets) {
  ...
  for (@target_urls) {
    if ($_ !~/(\=rang|\=repeat)/) {
	  ...
	  push @zhairiscans, $_ if $valido;
	  Print::Conclure($limit, $ifend, $isscan, $output, scalar @zhairiscans, scalar @targets) if $in eq $limit;
	}
  }
}

## END SCANS
Print::Conclure($limit, $ifend, $isscan, $output, scalar @zhairiscans, scalar @targets) if $limit eq 500;
